 ...
 * instances, so a report run never touches the replay stack. size comes from
 * BacktestConfig (validated at the driver boundary) and is mapped to the
 * strategy's quantity here — the driver never interprets strategy params.
 * Pure with respect to its inputs: the same candles and config produce an
 * identical BacktestResult every time (BacktestDriver's determinism
 * guarantee).
 */
